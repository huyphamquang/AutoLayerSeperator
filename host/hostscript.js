...
function gen_pa_mau(color_code, color_layer, text_layer, folders) {
    ...
    if (image_entry == null) {
        return confirmContinue("Không tìm thấy mã mầu " + color_code);
    }
    ...
    for (var i = 0; i < layers.length; i++) {
        ...
        switch (name) {
            ...
            case "anh_pa":
                ...
                if (file_entry == null) {
                    if (!confirmContinue('Không tìm thấy ảnh ' + (idx_pa + 1).toString() + ' trong thư mục ' + config.texture4_folder)) return false;
                }
            ...
            default:
                if (name.indexOf(prefix) == 0) {
                    ...
                    if (file_entry == null) {
                        if (!confirmContinue('Layer ' + name + ' Không tìm thấy ảnh ' + file_name)) return false;
                    }
                    else replaceImageToLayer(file_entry, layer, true);
                }
...
function createOutputFolders(config) {
    try {
        ...
        alert("Lỗi khi tạo thư mục output: " + err);
        return false;
    }
}
...
function separateLayers(config) {
    try {
        ...
        //printDebug("KeepOpen parameter: " + keepOpen);
        ...
        for (var i = 0; i < colorLayers.length; i++) {
            ...
            printDebug("Creating color range selection for: " + colorLayer.hex);
            doc.selection.deselect();
            selectColorRange(hexC(colorLayer.hex),hexC(colorLayer.hex));

            //var hasSelection = createColorRangeSelection(colorLayer.hex);
            //if (hasSelection) hasSelection = IsHasSelection(doc);
            
            if (!IsHasSelection(doc)) {
                printDebug("No selection found for color " + colorLayer.hex + " - skipping");
                continue; // Bỏ qua nếu không có vùng chọn
            }
            

            modifySelection(-2);
            // Kiểm tra sau khi thu hẹp - nếu không có selection thì đây là vùng nhiễu
            if (!IsHasSelection(doc)) {
                printDebug("No selection found after narrowing - this is noise, skipping");
                continue; // Bỏ qua nếu không có vùng chọn (nhiễu)
            }

            // Nếu có selection sau khi thu hẹp -> đây là layer bình thường
            // Phục hồi lại vùng chọn cũ trước khi thu hẹp
            printDebug("Clearing selection before restoring original selection");
            doc.selection.deselect();
            printDebug("Restoring original selection after noise filtering");
            selectColorRange(hexC(colorLayer.hex),hexC(colorLayer.hex));
            
            // Mở rộng vùng chọn 1 pixel
            printDebug("Expanding selection by 1 pixel");
            modifySelection(1);

            // Thực hiện hàm similar sau khi lọc nhiễu
            for (var m = 0; m < colorLayer.similar; m++) {
                var ok = similar();
                if (!ok) {
                    printDebug("'Similar' skipped/failed at iteration " + m + ", breaking loop");
                    break;
                }
                if (!IsHasSelection(doc)) {
                    printDebug("Selection absent after 'Similar' at iteration " + m + ", breaking loop");
                    break;
                }
            }
            ...
            layerTachLop.blendMode = BlendMode.MULTIPLY;
